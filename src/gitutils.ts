...
type gitServiceTypes = 'github' | 'gitlab' | 'bitbucket'

export async function downloadAndExtractRepo(gitService: string, user: string, repo: string, branch: string, token: string) {
    ...
    switch (gitService){
        case 'github':
            ...
            break
        case 'gitlab':
            url = `https://gitlab.com/api/v4/projects/${user}%2F${repo}/repository/archive.zip?sha=${branch}`
    ...
    try {
        ...
        try {
            ...
            const extractPath = `/tmp/${commitSha}_${user}_${repo}_${branch}`
            // Save zip
            ...
            const mainFolderPath = zipEntries.find((entry: any) => entry.isDirectory && entry.entryName.endsWith('/') &&  (entry.entryName.match(/\//g) || []).length == 1).entryName;
