...
(async () => {
    ...
    try {
        const nodesFile = await fs.readFile(`./tmp/graphNodes-${repoName}-${modelNoDots}.json`, 'utf-8')
        graphNodes = JSON.parse(nodesFile)
    
    
        const linksFile = await fs.readFile(`./tmp/graphLinks-${repoName}-${modelNoDots}.json`, 'utf-8')
        graphLinks = JSON.parse(linksFile)

        const documentedFolders = await generateDocumentation(graphNodes, graphLinks, repoName, model)

        fs.writeFile(`./tmp/graphNodes-${repoName}-${modelNoDots}.json`, JSON.stringify(graphNodes, null, 2))
        fs.writeFile(`./tmp/graphLinks-${repoName}-${modelNoDots}.json`, JSON.stringify(graphLinks, null, 2))
        fs.writeFile(`./tmp/graphFolders-${repoName}-${modelNoDots}.json`, JSON.stringify(documentedFolders, null, 2))
    } catch {
