...
//CAMBIOO
export async function generateFolderPrompts(
  nodes: GraphNode[],
  repoName: string,
  folderName: string,
  mostImportantNodesWithCode: string[],
  ...
  //console.log("Most used nodes per type", mostUsedNodesPerTypeString);

  let systemPrompt = `You are a helpful code expert and wikipedia editor who is writing a documentation for repository ${repoName}, which uses the following languages: ${allLanguagesString}.`;
  systemPrompt += `\nThese are the most common elements from the repository:\n${mostUsedNodesPerTypeString}\n\n`;
  systemPrompt += `\nThe following is the mapping between the id's with the most important nodes in the repository and the code of each important node:\n${mostImportantNodesWithCode}\n\n. Use this list when asked about usage examples.\n\n`;
  ...
  if (folderName.length === 0) {
    ...
    4. **Usage Examples**: Include examples of the most important elements: how to use them and what they do. Use the mapping given between the most important id's and their code in order to include precise usage examples.
    5. **Glossary**:  Definitions of key terms and concepts used in the project.
    It is very important to be concise and clear in your responses. Also, I am going to give you an example of what I mean by a final wiki. The following is part of a wiki for the AutoGPT AI Agent framework repository:\n ${sampleWiki}\n\n 
    Pay special attention to the style and structure of the wiki for AutoGPT AI Agent framework repository. You should follow a similar structure and style when generating the wiki for the repository you are working on (of course ignore the content, that will will change depending on the repository we want to document).`;
  } else {
  ...
  const folderContext =
    folderName.length > 0 ? `folder "${folderName}"` : `repository ${repoName}`;
  ...
  for (const fileNode of fileNodesInFolder) {
    if (fileNode.language === "markdown") {
      userPrompt += `If necessary, you can use the content of the following markdown file ${
        fileNode.label
      } to enhance the publication:\n${
        fileNode.generatedDocumentation ?? ""
      }\n`;
    } else {
      userPrompt += `Documentation for ${fileNode.language} file ${
        fileNode.label
      }:\n${fileNode.generatedDocumentation ?? ""}\n`;
    }
