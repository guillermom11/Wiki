...
export class Node {
      ...
      if (Object.keys(this.children).length > 0) {
        ...
        Object.values(this.children).forEach((n) => {
          if (classMethodInit && this.type === "class") {
            ...
            const isAssignment = ["assignment", "type", "enum"].includes(
              n.type
            );
  ...
  parseExportClauses(
    codebaseNodesMap: { [id: string]: Node },
    fileNodesMap: { [id: string]: Node }
  ) {
      ...
      switch (c.name) {
        ...
        case "name":
          ...
          node =
            ...
            codebaseNodesMap[`${moduleName}::${name}`];
          if (node) {
            ...
            const prevNode =
              this.children[node.id] || codebaseNodesMap[node.id];
  ...
  getChildrenDefinitions(): { [id: string]: Node } {
    if (!["file", "header"].includes(this.type) || this.language === "markdown")
      return {};
...
export class Codebase {
  ...
  simplify(attributes: string[] = []) {
    return Object.values(this.nodesMap).map((n) =>
      n.simplify(attributes, this.rootFolderPath)
    );
  }
  ...
  getLinks(): Link[] {
    ...
    for (const n of nodes) {
      if (n.parent) {
        ...
        links.push({
          source: n.parent.id.replace(this.rootFolderPath, ""),
      ...
      if (n.calls.length > 0)
        n.calls.forEach((c) => {
          links.push({
            source: n.id.replace(this.rootFolderPath, ""),
