...
createGraph.post('/', repoRequestValidator, async (c) => {
  ...
  const insertNodePromises = nodes.map(node => {
    ...
    VALUES (${nodeDBIds[node.id]}, ${repoId}, ${node.type}, ${node.language}, ${node.totalTokens}, ${node.documentation},
            ${node.code}, ${node.codeNoBody}, ${node.inDegree}, ${node.outDegree}, ${node.id}, ${node.label})
    `;
  });
